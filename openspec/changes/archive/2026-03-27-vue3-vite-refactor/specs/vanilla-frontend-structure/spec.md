...
**Reason**: The project adopts Vue 3 and Vite; production static hosting uses Vite build output. Serving only the raw repository tree without a build step is no longer sufficient for the SPA shell.

**Migration**: Contributors MUST run `npm install` and the documented build command (for example `npm run build`) before deploying; use the build output directory as the site root. Configure `base` for GitHub Pages project sites as documented in the repository.
